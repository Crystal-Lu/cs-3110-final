...
(** [full_column] checks if the column of the board is full raising Full and
    false if it's not full. Raises [Full] if the column is all occupied.
*)
...
(* [find_empty_spot st c] finds the lowest empty spot (in row number) of [c]th 
   column of the board. Basically, Empty that is closest to the end of the array 
   is the "lowest" empty spot. *)
...
let rec last_helper state column row =
  try
    (if (row>5) then 5 else
     ...
     if (state.board.(row).(column) <> Empty) then row else last_helper state 
         column (row-1))
...
(** [update_board st c player] updates the board. Remakes the board with new
    dot *)
...
let check_legal st column = 
  try
    (if (column < 0 || ((Array.length (current_board st)) < column)) then false
     else
       try
...
(* whoa this is really confusing sorry!
     ...
     I'm travelling. If it's positive, I increase the count and column by one 
     and run it again. If it's negative, I add 1 to count and subtract 1 from 
     column. If the current space isn't the player type, I check direction 
     again. If it's positive, I turn it negative and we go the other way now. 
     If it's negative, the loop ends here.
*)
...
let rec check_row st row (player:space) count col dir org = 
  try
    (if (count=4) then true else
     ...
     else if (dir > 0) then (check_row st row player count (org-1) (-1) org) 
     else if (dir < 0) then false else false)
...
let rec check_column st row player count col dir org =
  try
    (if (count = 4) then true else
     ...
     else if (dir > 0) then (check_column st (org-1) player count col (-1) org) 
     else false)
...
let rec check_left_diagonal st row player count col dir org_row org_col =
  try
    (if (count = 4) then true else
     if (row > 5 || col < 0) then (check_left_diagonal st (org_row-1) player
                                     count (org_col +1) (-1) org_row org_col) 
     else if (row < 0 || col > 6) then false
     else if (st.board.(row).(col) = player) then
       if (dir > 0) then
         (check_left_diagonal st (row+1) player (count+1) (col-1) 1 
            org_row org_col)
       else (check_left_diagonal st (row-1) player (count+1) (col+1) (-1) 
               org_row org_col)
     else if (dir > 0) then 
       (check_left_diagonal st (org_row-1) player count (org_col+1) (-1) 
          org_row org_col) 
     else false)
...
let rec check_right_diagonal st row player count col dir org_row org_col =
  try
    (if (count = 4) then true else
     if (row > 5 || col > 6) then 
       (check_right_diagonal st (org_row-1) player count (org_col-1) (-1) 
          org_row org_col) 
     else if (row < 0 || col < 0) then false 
     else if (st.board.(row).(col) = player) then
       if (dir > 0) then (check_right_diagonal st (row+1) player (count+1) 
                            (col+1) 1 org_row org_col) 
       else (check_right_diagonal st (row-1) player (count+1) (col-1) (-1)
               org_row org_col)
     else if (dir > 0) then (check_right_diagonal st (org_row-1) player count 
                               (org_col-1) (-1) org_row org_col) else false)
  with Invalid_argument(_) -> raise Right_Diagonal

(** [check_four] looks for four consecutive dots. *)
let check_four st n =
  try
    (if (check_row st (last_dot_placed st n)
           (st |> current_player |> player_to_space) 0 n 1 n) then true else
     if (check_column st (last_dot_placed st n) 
           (st |> current_player |> player_to_space) 0 n 1 
           (last_dot_placed st n)) then true
     else if (check_left_diagonal st (last_dot_placed st n) 
                (st |> current_player |> player_to_space) 0 n 1 
                (last_dot_placed st n) n) then true 
     else if (check_right_diagonal st (last_dot_placed st n) 
                (st |> current_player |> player_to_space) 0 n 1 
                (last_dot_placed st n) n) then true 
     else false)
  with Full ->
    (  if (check_row st 5 (st |> current_player |> player_to_space) 0 n 1 n) 
       then true else
       if (check_column st 5 (st |> current_player |> player_to_space) 0 n 1 5)
       then true else
       if (check_left_diagonal st 5 (st |> current_player |> player_to_space) 
             0 n 1 5 n) 
       then true else
       if (check_right_diagonal st 5 (st |> current_player |> player_to_space)
             0 n 1 5 n) 
