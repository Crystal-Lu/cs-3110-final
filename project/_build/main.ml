...
let rec play_game () : unit =
  ...
  ANSITerminal.(print_string [white] "\nPlease choose the number of players: 1 or 2. If you'd like to use the graphics module, please type 'GUI'. \n> ");
...
and game_one state ai =
  ...
  ANSITerminal.(print_string [white] ("\nPlease write the column of the move you wish to play.\n" ^ "Or, type quit if you'd like to stop the game.\n\n> "));
  if (ai) then 
    ...
    with
      ...
      |Quit ->
        ANSITerminal.(print_string [white] ("\nIt was nice having you! We hope you enjoyed our game.\n"));
        exit 0;
      |Place (n) -> 
        (ANSITerminal.(print_string [white] ("\nDot placed in column " ^ string_of_int(n) ^ ".\n\n"));
         ...
         with
           ...
           ANSITerminal.(print_string [white] ("Congrats, Player " ^ (Data.player_to_int (current_player state)) ^ " won!\n")); 
           ANSITerminal.(print_string [white] ("\nDo you want to play it again? Type 'y' to play again.: ")); 
           let input = read_line () in
    ...
    with
    |Malformed ->
      ANSITerminal.(print_string [white] ("\nSorry, your command was undecipherable!\n Please write the column in which you'd like to place to dot.\n\n"));
      game_two state
    |Full ->
      ANSITerminal.(print_string [white] ("\nSorry, the column you chose is full. Please choose another column.\n"));
      game_two state
    |Win ->
      ANSITerminal.(print_string [white] ("Congrats, Player " ^ (Data.player_to_int (current_player state)) ^ " won!"))
...
and game_two state = 
  ...
  try
    ...
    |Place (n) -> 
      (ANSITerminal.(print_string [white] ("\nDot placed in column " ^ string_of_int(n) ^ ".\n\n"));
       ...
       | Win -> ANSITerminal.(print_string [white] ("Congrats, Player " ^ (Data.player_to_int (current_player state)) ^ " won!\n"))); 
      ANSITerminal.(print_string [white] ("\nDo you want to play it again? Type 'y' to play again.: ")); 
      let input = read_line () in
      if (input) = "y" || (input) = "Y" then play_game ()
      else ANSITerminal.(print_string [white] ("\nIt was nice having you! We hope you enjoyed our game.\n"));
  ...
  |Win ->
    ANSITerminal.(print_string [white] ("Congrats, Player " ^ (Data.player_to_int (current_player state)) ^ " won!"))
(* TODO: need to make sure they can't start the game in the middle of another one. *)
